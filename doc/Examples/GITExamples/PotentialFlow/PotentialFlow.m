...
figure(4); FEMtrimesh(FEMmesh,sqrt(ux.^2+ uy.^2))
           xlabel('x'); ylabel('y'); zlabel('v=|grad u|')
           zlim([0 0.5])

figure(14); mesh(xx,yy,sqrt(ux_int.^2+ uy_int.^2))
            xlabel('x'); ylabel('y'); zlabel('v=|grad u|')
            zlim([0 0.5])

xx = linspace(0,5,101); yy = 0.25*ones(101,1);
[u_int,ux_int,uy_int] = FEMgriddata(FEMmesh,-u,xx,yy);
%%ux_int = FEMgriddata(FEMmesh,-ux,xx,yy); %% interpolate ux
figure(3); plot(xx,ux_int)
           xlabel('x'); ylabel('horizontal velocity');
           ylim([0 0.5])

figure(5)
%%if size(FEMmesh.elem,2)==6 FEMmesh = MeshQuad2Linear(FEMmesh); endif
FEMtricontour(FEMmesh,sqrt(ux.^2+ uy.^2),21)
