...
## -*- texinfo -*-
## @deftypefn{function file}{}@var{integral} = FEMIntegrate(@var{Mesh},@var{u})
##
...
##return value
##@itemize
##@item @var{intgeral} the numerical approximation of the integral
##@end itemize
##
...
function integral = FEMIntegrate(Mesh,u)
  ...
  if size(Mesh.elem,2)==3  % linear elements
    ...
    integral = sum(uV)*(Mesh.elemArea)/3;
    ...
    integral = 0;
    for ne = 1:nElem
      integral += 2*sum(w.*uV(:,ne))*Mesh.elemArea(ne);
    endfor
  endif
endfunction
