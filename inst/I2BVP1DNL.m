...
## -*- texinfo -*-
## @deftypefn{function file}{}[@var{x},@var{u},@var{t}] = I2BVP1DNL(@var{interval},@var{w2},@var{w1},@var{a},@var{b},@var{c},@var{d},@var{f},@var{BCleft},@var{BCright},@var{u0},@var{u1},@var{t0},@var{tend},@var{steps},@var{options})
##
##   solve a second order 1D initial boundary value problem (IBVP)
##
##    w2(x)*d^2/dt^2 u(x,t) + w1(x)*d/dt u(x,t) - (a(x)*u'(x,t))' + b(x)*u'(x,t) + c(x)*u(x,t) = d(x)*f(x,t,u)
##
##      with initial conditions u(x,t0) = u0(x) and d/dt u(x,t0) = u1
...
##@item @var{f = @@(x,t,u)} a function handle to evaluate the nonlinear function  f(x,t,u)
...
function  [x,u,t] = I2BVP1DNL(interval,w2,w1,a,b,c,d,f,BCleft,BCright,u0,u1,t0,tend,steps,varargin)
...
f_var = 1; %% f depends on time or u
if (length(f)>1)&&isnumeric(f)  %% f given as vector
  f_values = f;
  f_var = 0;  %% f does not depend on time or u
...
switch solver
  case 'IMPLICIT'
	...
	if f_var
	  switch BC  %% add the Dirichlet values at the endpoints
	    case 'DD'
	      u_t = [BCleft;u_new+uB;BCright];
	    case 'DN'
	      u_t = [BCleft;u_new+uB];
	    case 'ND'
	      u_t = [u_new+uB;BCright];
	    case 'NN'
	      u_t = u_new;
	  endswitch %% BC
	  f_values = f(x,tt,u_t);
