...
## -*- texinfo -*-
## @deftypefn{function file}{}@var{Tresca} = EvaluateTrescaAxi(@var{sigma_x},@var{sigma_y},@var{sigma_z},@var{tau_xz})
##
...
function sigma_T = EvaluateTrescaAxi(sigma_x,sigma_y,sigma_z,tau_xz)
  Discr = sqrt((sigma_x-sigma_z).^2+4*tau_xz.^2)/2;
  Sum = sigma_x+sigma_z;
  sigma_1 = Sum + Discr; sigma_2 = Sum - Discr;
  sigma_T = max([abs(sigma_1-sigma_2),abs(sigma_1-sigma_y),abs(sigma_2-sigma_y)],[],2);
endfunction
