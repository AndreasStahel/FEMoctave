...
DEFUN_DLD (FEMEquationComplex, args, ,
   "-*- texinfo -*-\n\
@deftypefn {} {} [@var{A},@var{b}] = FEMEquationComplex(@var{mesh},@var{a},@var{b0},@var{bx},@var{by},@var{f},@var{gD},@var{gN1},@var{gN2})\n\
\n\
sets up the system of linear equations for a numerical solution of a PDE\n\
using a triangular mesh with elements of order 1.\n\
Complex coefficinets are permitted.\n\
\n\
@verbatim\n\
...
if (args(8).is_string()) {  // function given as string
 ...
 Matrix M(2,3);
 ComplexMatrix elMat (3,3);
 ComplexMatrix A(2,2);
 ComplexColumnVector elVec (3);
 ColumnVector dofs (3);
 ...
 for(int k=0; k<elem.rows(); k++){ // loop over all elements
   ...
   if (isotropic ==1){
     // factor* M'*M
     Complex  factor = (aV(k3)+aV(k3+1)+aV(k3+2))/(12.0*area);
   ...
   for (int k1=0;k1<3;k1++){ // loop over corners
	 ...
	 if (dofs(k2)>0){ // variable value at this corner
	   ...
	   if (gDscalar){
	     gVec((int)dofs(k1)-1) += gDValue*elMat(k1,k2);
	   }
	   else {
	     //Matrix tmpCorners(1,2);
...
// test if there could be any contribution from edges
 ...
 if ((gN1scalar==false)||(abs(gN1Value)>0.0)||(gN2scalar==false)||(abs(gN2Value)>0.0)){
   //ComplexMatrix points(2,2);
   //points.fill((0.0,0.0));
   Matrix points(2,2);
   points.fill(0.0);
   ComplexColumnVector res1Vec(2);
   ComplexColumnVector res2Vec(2);
   ...
   for(int k = 0; k<edges.rows(); k++){ // loop over all edges
     if ((int)edgesT(k)==NEUMANN){  // work with all Neumann edges
       ...
       argin(0) = points;
       length =	 sqrt((corners(1,0)-corners(0,0))*(corners(1,0)-corners(0,0))+
	              (corners(1,1)-corners(0,1))*(corners(1,1)-corners(0,1)) )/2.0;
       dofs(0) = (int)n2d((int)edges(k,0)-1);
       dofs(1) = (int)n2d((int)edges(k,1)-1);

       // evaluate the values of gN1 at the two Gauss points
       if ((gN1scalar==false)||(abs(gN1Value)>0.0)){
	 ...
	 if (gN1scalar){
	   ...
	   //octave_stdout <<"res1Vec = "<< norm(res1Vec(0)) <<"\n";
	 }
	 ComplexColumnVector edgeVec(2);
	 edgeVec(0) = length * ((1.0-alpha)*res1Vec(0)+alpha*res1Vec(1));
	 edgeVec(1) = length * (alpha*res1Vec(0)+(1.0-alpha)*res1Vec(1));
	 ...
	 ComplexMatrix EdgeMat(2,2);
	 EdgeMat(0,0) = length*((1.0-alpha)*(1.0-alpha)*res2Vec(0)
			                     +alpha*alpha*res2Vec(1));
	 EdgeMat(0,1) = length*((1.0-alpha)*alpha*(res2Vec(0)+res2Vec(1)));
	 EdgeMat(1,0) = EdgeMat(0,1);
	 EdgeMat(1,1) = length*((1.0-alpha)*(1.0-alpha)*res2Vec(1)
			                     +alpha*alpha*res2Vec(0));
	 for (int k1=0;k1<2;k1++){
	   if(dofs(k1)>0){// variable value at this corner, generate equation
		 ...
		 if (gDscalar){
		   gVec((int)dofs(k1)-1)  -= gDValue*EdgeMat(k1,k2);
		 }
		 else {
		   ...
		   nargout  = 1;
		   res      = octave::feval (gDFunc, argin, nargout);
		   gVec((int)dofs(k1)-1) -= EdgeMat(k1,k2)*res(0).complex_value();
		 }// if gDscalar
	       }//else Dirichlet node
	     }// k2
	   }// dofs(k1)
